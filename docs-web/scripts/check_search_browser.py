...
def exercise_search(page, base_url, client=False):
    ...
    for query, has_results in (("Scene", True), ("Molecule", True), ("surface", True), ("from_sdf", True), ("cosmol_viewermissingsearchtoken", False), ("<script>alert(1)</script>", False)):
    ...
    page.locator('#docs-query').fill('surface')
    page.wait_for_function("document.querySelector('#search-results')?.dataset.query === 'surface'")
    page.locator('#docs-query').fill('Molecule')
    ...
    page.wait_for_selector('#docs-search-form[data-ready="true"]')
    assert page.locator('#search-results li').count() >= 1
    page.locator('#docs-query').fill('surface')
    ...
    for query in ('molecular surface', 'Molecule & atom', '\u5206\u5b50', '<script>alert(1)</script>'):
        ...
        with sync_playwright() as playwright:
            ...
            try:
                ...
                for legacy in ("/api", "/api.html", "/api/"):
                    ...
                    page.goto(base_url + legacy + "?from=legacy#cosmol_viewer.Molecule")
                    assert page.url == base_url + "/python/api?from=legacy#cosmol_viewer.Molecule"
                    assert page.locator('[id="cosmol_viewer.Molecule"]').count() == 1
...
def exercise_listener_cleanup(page, base_url):
    ...
    page.goto(base_url + '/python/api#cosmol_viewer.Molecule.stick')
    page.wait_for_function("anchorListeners.length > 0 && anchorListeners.every(item => !item.active)")
    page.wait_for_function("""() => {
        const y = document.getElementById('cosmol_viewer.Molecule.stick').getBoundingClientRect().top;
...
def exercise_navigation(page, base_url):
    anchor = 'cosmol_viewer.Molecule.stick'
