...
class DeploymentTests(unittest.TestCase):
    ...
    def page(self, relative, route, runtime=False):
        ...
        title = f"COSMol Viewer {route or 'Documentation'}"
        description = f"Read the COSMol Viewer {route or 'documentation'} guide and reference."
        robots = "noindex, follow" if route in EXCLUDED_ROUTES else "index, follow"
        meta = {
            "description": description, "robots": robots,
            "og:type": "website", "og:site_name": "COSMol Viewer Documentation",
        ...
        html += '<nav class="cosmol_viewer-project-links">'
        ...
        if runtime:
            ...
            html += '<link rel="preload" as="script" href="/assets/cosmol-viewer-docs-web.js">'
            html += '<script type="module" src="/assets/cosmol-viewer-docs-web.js"></script>'
        html += '</body></html>'
    ...
    def prepare(self):
        ...
        shutil.copyfile(
            docs_web.parent / "crates/python/docs/source/robots.txt", self.public / "robots.txt"
        )
        shutil.copyfile(
            docs_web.parent / f"crates/python/docs/source/{INDEXNOW_KEY}.txt", self.public / f"{INDEXNOW_KEY}.txt"
        )
    ...
    def test_output_rejects_duplicate_titles_and_descriptions(self):
        ...
        original = path.read_text(encoding="utf-8")
        for old, new, message in (
            ("COSMol Viewer python/installation", "COSMol Viewer python/api", "duplicates.*title"),
            ("Read the COSMol Viewer python/installation guide and reference.", "Read the COSMol Viewer python/api guide and reference.", "duplicates.*description"),
        ):
    ...
    def test_output_requires_project_links_on_copied_module_pages(self):
        ...
        module = self.page("python/_modules/index.html", "python/_modules/")
        self.assertEqual(check_output(self.public), 18)
        module.write_text(module.read_text().replace("cosmol_viewer-project-links", "other-links"), encoding="utf-8")
    ...
    def test_social_image_download_validates_and_saves_entire_response(self):
        ...
        with patch("prepare_deployment.urlopen") as fetch:
            ...
            self.assertEqual(request.get_header('User-agent'), 'COSMol-viewer-Docs-CI/1.0')
    ...
    def test_internal_links_must_resolve_without_legacy_redirects(self):
        self.prepare()
        path = self.public / "python/molecules.html"
        original = path.read_text(encoding="utf-8")
