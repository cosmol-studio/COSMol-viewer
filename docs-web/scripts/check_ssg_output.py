...
INDEXNOW_KEY = "1d047fb8-319c-41ca-8fa0-3955d72ece0f"
PROJECT_LINKS = {
    "https://github.com/cosmol-studio/COSMol-viewer",
    "https://crates.io/crates/cosmol_viewer",
    "https://crates.io/crates/cosmol_viewer_core",
    "https://docs.rs/cosmol_viewer/latest/cosmol_viewer/",
    "https://pypi.org/project/cosmol-viewer/",
    BASE_URL,
...
def local_resource(public: Path, url: str) -> Path:
    parsed = urlsplit(url)
    if parsed.netloc and (parsed.scheme != "https" or parsed.netloc != "viewer.cosmol.org"):
...
def check_page_metadata(page, path: Path, route: str) -> tuple[str, str]:
    ...
    if "COSMol Viewer" not in title:
        raise ValueError(f"{path.name}: title does not identify COSMol Viewer")
    if page.h1_count != 1:
    ...
    expected_meta = {
        "og:type": "website",
        "og:site_name": "COSMol Viewer Documentation",
        "og:title": title,
...
def check_output(public: Path) -> int:
    ...
    for route in ("", *ROUTES):
        ...
        if any(marker in html for marker in (
            "cosmol-viewer-docs-web", "hydrate_queue", "initial_dioxus_hydration_data",
        )):
