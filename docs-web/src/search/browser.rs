    ...
    fn render(&self, query: &str) -> Result<(), JsValue> {
        ...
        *self.results.borrow_mut() = self.index.search(query);
        ...
        let message = if query.is_empty() {
            ...
            format!("No results for “{query}”. Try Scene, surface, or from_sdf.")
        } else {
...
pub fn mount_search() -> Result<(), JsValue> {
    ...
    let initial = js_sys::Reflect::get(window.as_ref(), &"cosmol_viewerInitialSearch".into())?;
    if !initial.is_undefined() && !initial.is_null() {
        ...
        js_sys::Reflect::delete_property(
            ...
            &"cosmol_viewerInitialSearch".into(),
        )?;
    }
