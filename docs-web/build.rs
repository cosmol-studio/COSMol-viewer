...
fn main() {
    println!("cargo:rerun-if-changed=../crates/python/docs/source");
    println!("cargo:rerun-if-changed=../crates/python/docs/_build/html");
    ...
    println!("cargo:rerun-if-env-changed=COSMOL_VIEWER_WASM_BINDGEN");
    println!("cargo:rerun-if-env-changed=COSMOL_VIEWER_DOCS_PYTHON");

    let manifest_dir =
        PathBuf::from(env::var_os("CARGO_MANIFEST_DIR").expect("manifest directory"));
    let repository_root = manifest_dir.parent().expect("repository root");
    let docs_source = repository_root.join("crates/python/docs/source");
    let docs_output = repository_root.join("crates/python/docs/_build/html");
...
fn docs_python(repository_root: &Path) -> PathBuf {
    env::var_os("COSMOL_VIEWER_DOCS_PYTHON")
        .map(PathBuf::from)
        .filter(|path| path.is_file())
        .or_else(|| {
            let path = repository_root.join(if cfg!(windows) {
                "docs-web/.venv/Scripts/python.exe"
            } else {
                "docs-web/.venv/bin/python"
            });
...
fn generate_document_module(output: &Path) -> String {
    ...
    for relative in ["_static/pygments.css", "_static/cosmol_viewer.css"] {
...
mod tests {
    ...
    fn preserves_nested_api_toc_verbatim() {
        let toc = "\n<ul><li><a href=\"#cosmol_viewer.Molecule\">Molecule</a><ul><li><a href=\"#cosmol_viewer.Molecule.add_hydrogens_\"><code>add_hydrogens_()</code></a></li></ul></li></ul>\n";
        let html =
    ...
    fn generates_page_mapping_with_empty_fallback() {
        ...
        let toc = "<ul><li><a href=\"api.html#cosmol_viewer.Atom\">Atom</a></li></ul>";
        ...
        for page in pages() {
            let expected = if page == "api" {
                "<ul><li><a href=\"/python/api#cosmol_viewer.Atom\">Atom</a></li></ul>"
            } else {
                ""
            };
